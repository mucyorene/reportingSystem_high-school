<?php
  ...
  $tid=$_SESSION['deanp'];
  $sqli=mysqli_query($conn,"SELECT *FROM dean WHERE id='$tid'") or die(mysqli_error());
  $rowtitle=mysqli_fetch_array($sqli);
...
$stdsel=mysqli_query($conn,"SELECT students.*,reported.* FROM students,reported WHERE 
  students.s_id=reported.reporter_id order by reported.reporter_id desc") or die(mysqli_error());
$count=1;
while ($std=mysqli_fetch_array($stdsel)) {
?>
<tr>
  <td><?= $count++;?></td>
...
$stdsel=mysqli_query($conn,"SELECT students.*,comments.* FROM students,comments WHERE 
  students.s_id=comments.commenter_id order by comments.com_id desc") or die(mysqli_error());
$coco=1;
while ($com=mysqli_fetch_array($stdsel)) {
?>
<tr>
<td><?= $coco++;?></td>
...
<?php
$so=mysqli_query($conn,"SELECT *FROM classes");
?>
<table border="1">
<tr>
<td>#&nbsp</td>
...
$var=mysqli_real_escape_string($conn,$vars);
$searchquery=mysqli_query($conn,"SELECT * FROM students WHERE firstname like '%$var%'
OR lastname like '%$var%'") or die(mysqli_error());
...
$m=mysqli_query($conn,$qrys);
$rows=mysqli_num_rows($m);
?>
<a href=""></a>
</table>
...
while ($rowa=mysqli_fetch_array($sql1)) {
   if ($u%2==0) {
    $color="#3d5457";
  }else{
    $color="#49676b";
...
$k=$a.date("Y");
$cv=$_POST['classe'];
$sq=mysqli_query($conn,"SELECT *FROM classes WHERE classname='$cv'");
if ($sq) {
$row=mysqli_fetch_array($sq);
$cl_id=$row['cl_id'];
}
$sqlupdate=mysqli_query($conn,"UPDATE students SET 
  firstname='$a',
  lastname='$b',
  gender='$c',
  DOB='$d',
  ...
  fathername='$g',
  mathername='$h',
  email='$i' WHERE s_id='$ids'") or die(mysqli_error());
if ($sqlupdate) {
echo "updated";
}
else{
...
$teacher=$_POST['ide'];
$aa=mysqli_real_escape_string($_POST['fname1']);
$bb=mysqli_real_escape_string($_POST['lname']);
$cc=mysqli_real_escape_string($_POST['gender']);
$dd=mysqli_real_escape_string($_POST['idcard']);
$ee=mysqli_real_escape_string($_POST['location']);
$ff=mysqli_real_escape_string($_POST['phone1']);
$gg=mysqli_real_escape_string($_POST['qualify']);
$hh=mysqli_real_escape_string($_POST['mail']);
$teacherup=mysqli_query("UPDATE teachers SET
 firstname='$aa',
lastname='$bb',
gender='$cc',
idnumber='$dd',
...
qualification='$gg',
email='$hh'
  WHERE t_id='$teacher'") or die(mysqli_error());
if ($teacherup) {
echo "updated";
}else{

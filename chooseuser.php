...
$tid=$_SESSION['deanp'];
$sqli=mysqli_query($conn,"SELECT *FROM dean WHERE id='$tid'") or die(mysqli_error($conn));
...
<table border="0" style="margin-left:20%;">
    ...
    <tr>
		...
		<select name="gender" required title=$conn,"Select gender">
...
while ($loa=mysqli_fetch_array($dis)) {
?>
...
<?php
if (isset($_POST['save'])) {
$a=mysqli_real_escape_string($_POST['fname1']);
$b=mysqli_real_escape_string($_POST['lname']);
$c=mysqli_real_escape_string($_POST['gender']);
$d=mysqli_real_escape_string($_POST['idcard']);
$e=mysqli_real_escape_string($_POST['location']);
$f=mysqli_real_escape_string($_POST['phone1']);
$g=mysqli_real_escape_string($_POST['qualify']);
$h=mysqli_real_escape_string($_POST['mail']);
$i=$b;
$j=$a.date("y");
$checi=mysqli_query($conn,"SELECT * FROM teachers WHERE firstname='$a' AND lastname='$b' AND idnumber='$d'") or die(mysqli_error());
if (mysqli_num_rows($checi)>0) {
...
if (isset($_POST['saves'])) {
	$a=mysqli_real_escape_string($conn,$_POST['fname1']);
	$b=mysqli_real_escape_string($conn,$_POST['lname']);
	$c=mysqli_real_escape_string($conn,$_POST['gender']);
	$d=mysqli_real_escape_string($conn,$_POST['sel']."-".$_POST['sele']."-".$_POST['select']);
	$e=mysqli_real_escape_string($conn,$_POST['location']);
	$f=mysqli_real_escape_string($conn,$_POST['phone']);
	$g=mysqli_real_escape_string($conn,$_POST['fname']);
	$h=mysqli_real_escape_string($conn,$_POST['mname']);
	$i=mysqli_real_escape_string($conn,$_POST['emails']);
	$l=mysqli_real_escape_string($conn,$_FILES['pic']['name']);
	$j=$b;
	$k=$a.date("Y");
	$cv=mysqli_real_escape_string($conn,$_POST['classe']);
$checkid=mysqli_query($conn,"SELECT * FROM students WHERE firstname='$a' AND lastname='$b' AND phones='$f'") or die(mysqli_error());
...
$sql=mysqli_query($conn,"INSERT INTO students VALUES
(' ','$a','$b','$c','$d','$e','$f','$g','$h','$i','$j','$k','$cl_id','$l','check')") or die(mysqli_error($conn));
